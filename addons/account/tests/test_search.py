...
class TestSearch(TransactionCase):
    ...
    def test_name_search(self):
        ...
        self.assertEqual({self.atax[0]}, set([a[0] for a in atax_ids]), "name_search 'ilike Tax' should have returned Tax Received account only")

        atax_ids = self.account_model.name_search(cr, uid, name="Tax", operator='not ilike', args=[('id', 'in', self.all_ids)])
        self.assertEqual({self.apurchase[0], self.asale[0]}, set([a[0] for a in atax_ids]), "name_search 'not ilike Tax' should have returned all but Tax Received account")

        apur_ids = self.account_model.name_search(cr, uid, name='1101', operator='ilike', args=[('id', 'in', self.all_ids)])
        self.assertEqual({self.apurchase[0]}, set([a[0] for a in apur_ids]), "name_search 'ilike 1101' should have returned Purchased Stocks account only")

        apur_ids = self.account_model.name_search(cr, uid, name='1101', operator='not ilike', args=[('id', 'in', self.all_ids)])
        self.assertEqual({self.atax[0], self.asale[0]}, set([a[0] for a in apur_ids]), "name_search 'not ilike 1101' should have returned all but Purchased Stocks account")

        asale_ids = self.account_model.name_search(cr, uid, name='200 Sales', operator='ilike', args=[('id', 'in', self.all_ids)])
        self.assertEqual({self.asale[0]}, set([a[0] for a in asale_ids]), "name_search 'ilike 200 Sales' should have returned Product Sales account only")

        asale_ids = self.account_model.name_search(cr, uid, name='200 Sales', operator='not ilike', args=[('id', 'in', self.all_ids)])
        self.assertEqual({self.atax[0], self.apurchase[0]}, set([a[0] for a in asale_ids]), "name_search 'not ilike 200 Sales' should have returned all but Product Sales account")

        asale_ids = self.account_model.name_search(cr, uid, name='Product Sales', operator='ilike', args=[('id', 'in', self.all_ids)])
        self.assertEqual({self.asale[0]}, set([a[0] for a in asale_ids]), "name_search 'ilike Product Sales' should have returned Product Sales account only")

        asale_ids = self.account_model.name_search(cr, uid, name='Product Sales', operator='not ilike', args=[('id', 'in', self.all_ids)])
        self.assertEqual({self.atax[0], self.apurchase[0]}, set([a[0] for a in asale_ids]), "name_search 'not ilike Product Sales' should have returned all but Product Sales account")
